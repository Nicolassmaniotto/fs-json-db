...
import { findItemByFunction } from './findByFunction.js'
...
async function findItemInAll(dir, findVar, basesParam = null, typeFind = '1') {
    ...
    try {
        ...
        } else if (typeFind.toLowerCase() == 'func' || typeFind.toLowerCase() == 'function' || typeFind == '4') {
            if (typeof (findVar) !== "function") throw "not function"
            for (var i = pos; i <= comprimento; i++) {
                await findItemByFunction(`${dir}/${i}.jsonl`, findVar, bases).then((result) => {
                    // console.log(result)
                    if (result != 0 && result != null) {
                        for (let j in result) {
                            if(typeof(result[j])==="object"){
                                ...
                                let id_i = parseInt(parseFloat(i * bases.qtId) - bases.qtId)+1
                                item = item.concat({id:id_i,item:result[j]})
                            }
